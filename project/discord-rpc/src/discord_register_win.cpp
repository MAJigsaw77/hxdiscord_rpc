...
static HRESULT StringCbPrintfW(LPWSTR pszDest, size_t cbDest, LPCWSTR pszFormat, ...)
{
	...
	cbDest /= 2; // Size is divided by 2 to convert from bytes to wide
		     // characters - causes segfault othervise
	ret = vsnwprintf(pszDest, cbDest, pszFormat, va);
	pszDest[cbDest - 1] = 0; // Terminate the string in case a buffer
				 // overflow; -1 will be returned
	va_end(va);
	return ret;
}
#else
...
static void Discord_RegisterW(const wchar_t *applicationId, const wchar_t *command)
{
	...
	// Update the HKEY_CURRENT_USER, because it doesn't seem to require
	// special permissions.
